...
| §4 "every field read off mainnet" | mint facts, feed ids and depth figures were read on 2026-09-12 and 2026-09-15. Since 2026-09-23 thirteen Pyth accounts are pinned: Pyth's **shard 1** of the push oracle (`PDA([1u16 LE, feed id])`) keeps the share feeds of SPY, QQQ, NVDA, MSFT, GOOGL, TSLA, META, AAPL, AMZN and MSTR fresh to the second, while shard 0 is weeks stale for all of them. CRCL, COIN and HOOD have no fresh account in shards 0–7, so they are `unpriced`: on the program's table and on `/assets`, not offered on the rule page | The keeper reads pinned accounts and falls back to Hermes, which refuses equities below the Pro tier — so before the pins, a rule on any single name would have waited forever. A pin is the difference between an asset that settles and one that only waits |
...
| the docs say a keeper "cannot redirect the output" and its "only reward is the tip" | `finish_sweep` checks that the owner's asset account gained at least the Pyth-bounded minimum (price net of confidence, less the owner's tolerance) — not that the whole slice was delivered. A keeper that delivers only the minimum keeps the difference: about the tolerance plus Pyth's band, plus any move inside the 600 s a price stays valid. Scrip's keeper quotes the whole slice with the owner's account as the destination, so its fills sit at Pyth (the first two: +0.04%, −0.12%). `/security`, `/keepers`, `/docs/keepers` and the README now say exactly this | Found on 2026-09-23 by an outside review, confirmed in `lib.rs`. The fix — the Jupiter instruction's input amount and destination checked through the instructions sysvar, and a tighter age for keeper-posted prices — needs a program upgrade, which waits until after Stocklana: a public page that claimed more than the program enforces was the part that could not wait |
| the front door and `/company` say a stock "arrives at 3am on a Sunday" | "before the opening bell". Pyth's `Equity.US.SPY/USD` is published on weekdays, pre-market included (a sweep settled against it at 06:04 ET), and not at weekends; `Metal.XAU/USD` keeps market hours too. The ghost stub now reads "waiting for a price" instead of "a keeper is racing" whenever the keeper reports it, on the front door as well, and the try-it box promises "within seconds" only while a pinned price account would pass `finish_sweep`'s checks (`src/lib/pyth/ready.ts`) | A judge who scans the code on a Saturday would have read "within seconds" and "a keeper is racing" over money that could not settle until Monday |
| a claim is sponsored whenever the relayer can pay | sponsored only when the Payout's payer is in `SPONSOR_PAYERS` (`src/lib/relayer.ts`); any other payer's recipient claims at their own cost, and the claim button switches to that on the relay's `selfPay` answer | A sponsored claim pays rents the claimer can reclaim by closing the register and the token account, so an open sponsor was a faucet: about 0.005 SOL a round, bounded only by the relayer's balance. Found by an outside review on 2026-09-23 |
| a receipt shows the price it settled against | it also shows what the slice paid per unit against that price (`Filled at $768.64 a share · 0.04% above Pyth`), how long after the money landed the stock arrived (from the attributed arrival's block time), and what the sweep took from the float, split into rent, tip and any new token account — `src/lib/receipt/figures.ts`, tested on the first two mainnet sweeps. `/docs/pyth` shows the same figures across every receipt, and the pinned accounts' ages right now | These are the numbers a judge would compute by hand from the explorer. The first sweep honestly reads 4 min 28 s, from the day the keeper was being fixed |
| the keeper sends a sweep once and waits for its blockhash | it re-broadcasts the same signed bytes every poll until the network has seen them (`confirmSignature(…, resend)`), at 100,000 microlamports instead of 20,000 | The second mainnet sweep took 50 s from arrival to stock because its first transaction was dropped and the keeper waited out the blockhash: 40 of those seconds were a transaction nobody re-sent |
| every count on the floor includes Scrip's own wallets | `src/lib/team.ts` declares them (the founder, @scrip, both keepers, the relayer, and two wallets that claimed during testing); their stubs read "Scrip · team", `/security` lists them, and the floor and ledger count receipts to wallets outside the team | A judge should not have to work out from addresses that every receipt so far is the founder's; a count of people that quietly includes the builders is the one number that would discredit the rest |
